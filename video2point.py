...
if __name__ == '__main__':
    ...
    parser.add_argument('--json', action='store_true', help='generate json files', default=False)
    parser.add_argument('--pointcloud', action='store_true', help='generate point cloud', default=False)
    args = parser.parse_args()
    
    valid_cam = [int(c) for c in args.valid_cam.split(',')]
    
    assert args.extract or args.json or args.pointcloud, "At least one of --extract, --json, --pointcloud should be set"
    ...
    if args.json:
        ...
        if os.path.exists(train_output_path) and os.path.exists(test_output_path) and len(valid_cam) == 21:
            assert args.pointcloud == False and args.extract == False, "The transforms files already exist, please delete them if you want to regenerate them."
